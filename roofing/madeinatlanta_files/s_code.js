/* ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
* 
* SiteCatalyst code version: H.24.3.
* Copyright 1996-2012 Adobe, Inc. All Rights Reserved
* More info available at http://www.omniture.com 
* 
* ----------------------
* TABLE OF CONTENTS    
* ----------------------
* 
* Documentation: http://microsite.omniture.com/t2/help/en_US/sc/implement/oms_sc_implement.pdf
* ------------------------------------------------------------
* 
* 01.00 Configuration
* 02.00 Reusable Functions
* 03.00 Tracking Functions
* 04.00 Page Inialization
* 05.00 Omniture Proprietary Code
* 
* ------------------------------------------------------------
* Omniture Tracking Guide
* ------------------------------------------------------------
* s.pagename   Page Name // Final Output Example: (ONSTAR CONNECTIONS | SECTION | SUB-SECTION | etc.)
* s.channel    Section (Category)
* 
* s.Prop1      Persona
* s.Prop2      Page Name w/ Persona 
* s.Prop3      Postal Code 
* s.Prop4      Persona Changed To…
* s.Prop17     List of Contractors in Results
* s.Prop28     Compare Shingles - Products  

 
* eVar8        Video Name
* eVar4        Persona Changed From…
* eVar5        Persona Changed To…
* eVar10       Internal Search Results
* eVar11       ZIP Code Changed From…
* eVar12       ZIP Code Changed To…
* eVar13       Share Platform  
* eVar14       Shared Content  
* eVar15       Name of Form  
* eVar16       Find a  Contractor - ZIP Code
* eVar20       Contact / Call Type  
* eVar21        Register a Warranty - Step # and Name  
* eVar22        File a Warranty Claim - Step # and Name
* eVar23        Name of Contractor contacted
* eVar24        Order a Digital Sample - Product Name
* eVar26        Browse Activity
* eVar27        Product Name
* eVar28        Browse Activity | Product Name
* eVar31        On-Site Search Term  
* eVar32        Results?  ("Y" or "N")
* eVar33        Search Refine Type  
* eVar34        Sort By selection  
* eVar36        Product Type selection  
* eVar37        Result Link Name
* eVar38        On-Site Search Term | Result Link Name
* eVar71        Postal Code


* 

* Event4       Persona Selected
* Event8       Video Start  
* Event9       Video 50%  
* Event10      Video Complete  
* Event11      ZIP Code Changed
* Event12      Sign Up / Register
* Event13      Share
* Event15      Form Start  
* Event16      Form Complete   
* Event17      Find a Contractor
* Event18      Design Your Own Roof
* Event19      Order a Digital Sample
* Event20      Click to Call
* Event21      Warranty Event
* Event22      Warranty Event (Unique)  * Event23      Click to Reveal Phone Number
* Event26      Browse Shingles Event
* Event27      Browse Shingles Event (Unique)   
* Event28      Compare Shingles
* Event25      Select Color
* Event31      On-Site Search
* Event32      On-Site Search Results Click-Through
* Event33      Sort Search Results
* Event71      Storm Banner Impressions
* Event72      Storm Banner Click-Through
* Event41      Contact Events (Unique)* Event44      Click to Reveal Phone Number(Unique)* Event45 Click to Call (Unique)* Event46      E-Mail Contractor* Event47      E-Mail Contractor (Unique)* Event51      Locate Events (Unique)* Event52      Find a Contractor (Unique)* Event53      Find a Retail Store(Unique)* Event54      Locate a Professional Distributor (Unique)* Event61 Misc Events (Unique)* Event64      Help Green My City (Unique)* Event66      Help Green This Contractor (Unique)* Event65      Contractor Rewards Application (Unique)::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: */



/* ------------------------------------------------------------
* 01.00 Configurations
* ------------------------------------------------------------
* 1.01 Account Info
* 1.02 Tracking
* 1.03 Event Handling
* 1.04 Initial Page Config
* --------------------------------------------------------- */


// -----------------
// 1.01 Account Info
// -----------------

var s_account = 'owenscorningprod';
var s = s_gi(s_account);


// -------------
// 1.02 Tracking
// -------------

s.charSet = 'UTF-8';

// Conversion Config
s.currencyCode = 'USD';

// Link Tracking Config 
s.trackDownloadLinks = true;
s.trackExternalLinks = true;
s.trackInlineStats = true;
s.linkDownloadFileTypes = 'exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx';
s.linkInternalFilters = 'owenscorning.com,javascript:';
s.linkLeaveQueryString = false;
s.linkTrackVars = 'eVar4,eVar5,eVar8,eVar10,eVar11,eVar12,eVar13,eVar14,eVar15,eVar16,eVar20,eVar21,eVar22,eVar23,eVar24,eVar26,eVar27,eVar28,eVar31,eVar32,eVar33,eVar34,eVar36,eVar37,eVar38,eVar48,eVar49,eVar50,eVar71,evar62,events';
s.linkTrackEvents = 'event4,event8,event9,event10,event11,event12,event14,event13,event15,event16,event17,event18,event19,event20,event21,event22,event23,event24,event28,event25,event26,event27,event31,event32,event33,event41,event42,event43,event44,event45,event46,event47,event48,event49,event51,event52,event53,event54,event61,event62,event63,event64,event65,event71,event72,event66';
s.dynamicAccountList = 'owenscorningstaging=oc-roofing-app.stagin.owenscorning.com;owenscorningdev=oc-roofing.dev.owenscorning.com;owenscorningprod=www.owenscorning.com';
s.dynamicAccountMatch = window.location.host;
s.dynamicAccountSelection = true;
s.linkLeaveQueryString = false;


/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
//s.trackingServer = 'gmonstar.112.2o7.net';


// -------------------
// 1.03 Event Handling
// -------------------

var events = new Array();

events["Persona Selected"] = "event4";
events["Video Start"] = "event8";
events["Video 50%"] = "event9";
events["Video Complete"] = "event10";
events["ZIP Code Changed"] = "event11";
events["Find a Retail Store"] = "event12";
events["Locate a Professional Distributor"] = "event14";
events["Share"] = "event13";
events["Form Start"] = "event15";
events["Form Complete"] = "event16";
events["Find a Contractor"] = "event17";
events["Design Your Own Roof"] = "event18";
events["Order a Digital Sample"] = "event19";
events["Click to Call"] = "event20";
events["Request a Quote"] = "event24";
events["Compare Shingles"] = "event28";
events["Select Color"] = "event25";
events["On-Site Search"] = "event31";
events["On-Site Search Results Click-Through"] = "event32";
events["Sort Search Results"] = "event33";
//events["Storm Banner Impressions"] = "event37";
//events["Storm Banner Click-Through"] = "event38";



//added by noopur
events["Click to Reveal Phone Number"] = "event23";
events["Click to Reveal Phone Number(Unique)"] = "event44";
events["Contact Events (Unique)"] = "event41";
events["Misc Events (Unique)"] = "event61"
events["Contractor Rewards Application (Unique)"]="event65"
events["Warranty Event"] = "event21";
events["Warranty Event (Unique)"] = "event22";
events["Click to Call (Unique)"]="event45"
events["E-Mail Contractor"] = "event46";
events["E-Mail Contractor (Unique)"] = "event47";
events["Help Green My City (Unique)"] = "event64";
events["Help Green This Contractor (Unique)"] = "event66";
events["Locate Events (Unique)"] = "event51";
events["Find a Contractor (Unique)"] = "event52";
events["Find a Retail Store(Unique)"] = "event53";
events["Locate a Professional Distributor(Unique)"] = "event54";
events["Request a Quote (numeric)"]="event42";
events["Request a Quote (Unique)"]="event43";
events["PDF Downloads"] = "event62";
events["PDF Downloads (Unique)"] = "event63";
events["Product Details Event"] = "event48";
events["Product Details Event (Unique)"] = "event49";



//end by noopur
//start_mayank
events["Browse Shingles Event"] = "event26";
events["Browse Shingles Event (Unique)"] = "event27";
events["Storm Banner Impressions"] = "event71";
events["Storm Banner Click-Through"] = "event72";
//end_mayank
/* ------------------------------------------------------------
* 02.00 Reusable Functions
* --------------------------------------------------------- */

/* ------------------------------------------------------------
* 02.01 sDataReset
* ------------------------------------------------------------
* DESC: Resets all evars, sprops, and events
* --------------------------------------------------------- */

function sDataReset() {

    //s.pageName    = "ONSTAR CONNECTIONS | LANDING";
    s.pageName = null;
    s.channel = null;
    s.events = null;
    s.prop1 = null;
    s.prop2 = null;
    s.prop3 = null;
    s.prop4 = null;
    s.prop17 = null;
    s.prop24 = null;
    s.prop28 = null;
    //s.prop7           = null;
    //s.prop10          = null;
    //s.prop25          = "ONSTAR CONNECTIONS";
    s.campaign = null;
    s.events = null;
    s.eVar8 = null;
    s.eVar4 = null;
    s.eVar5 = null;
    s.eVar10 = null;
    s.eVar11 = null;
    s.eVar12 = null;
    s.eVar13 = null;
    s.eVar14 = null;
    s.eVar15 = null;
    s.eVar16 = null;
    s.eVar17 = null;
    s.eVar18 = null;
    s.eVar19 = null;
    s.eVar20 = null;
    s.eVar21 = null;
    s.eVar22 = null;
    s.eVar23 = null;
    s.eVar24 = null;
    s.eVar26 = null;
    s.eVar27 = null;
    s.eVar28 = null;
    s.eVar31 = null;
    s.eVar32 = null;
    s.eVar33 = null;
    s.eVar34 = null;
    s.eVar36 = null;
    s.eVar37 = null;
    s.eVar38 = null;
    s.eVar48 = null;
    s.eVar49 = null;
    s.eVar50 = null;
    s.eVar71 = null;
    s.eVar62 = null;
}


/* ------------------------------------------------------------
* SPECIAL CHARCTER WHITELIST
*
* string.replace(/[^\w\s|]/gi, '') 
* // where \w ,  \s,  | are allowed characters
*
* ------------------------------------------------------------



/* ------------------------------------------------------------
* 03.00 Tracking Functions
* ------------------------------------------------------------
* 03.01 trackLink
* 03.02 trackPersonaSwitch
* 03.03 trackZipCodeChangeEvent
* 03.04 trackCompareShingles
* 03.05 trackOnSiteSearchEvent
* 03.06 trackForm
* 03.07 trackOrderASampleEvent
* 03.08 trackWarrantyEvents
* 03.09 trackShingleColorEvent
* 03.10 trackSearchResultRefiningEvent
* 03.11 
* --------------------------------------------------------- */



/* ------------------------------------------------------------
* 03.01 trackLink
* ------------------------------------------------------------
* DESC: General link track
* --------------------------------------------------------- */

function trackLink(linkName, lnk) {

    s_linkName = linkName;
    s_linkType = "e";

	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    // eventName = events[linkName];

    // if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call

}

/* ------------------------------------------------------------
* 03.02 trackPersonaSwitch
* ------------------------------------------------------------
* DESC: Tracks the persona switching event
* --------------------------------------------------------- */
function trackPersonaSwitch(linkName, lnk, userPersona) {
    //alert("In trackLink");
    s_linkName = linkName;
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    //alert('Persona changes from ' + jQuery.cookie("category"));
    //alert('Persona changed to ' + userPersona);

    if (jQuery.cookie("category") == null) {
        s.eVar4 = "none";
    }
    else {
        s.eVar4 = jQuery.cookie("category"); //"Persona changed from"
    }

    s.eVar5 = userPersona; //"Persona changed to

    eventName = events[linkName]; // linkName will hold "Persona Selected"

    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call

}

/* ------------------------------------------------------------
* 03.03 trackZipCodeChangeEvent
* ------------------------------------------------------------
* DESC: Tracks the zip code change event
* --------------------------------------------------------- */
function trackZipCodeChangeEvent(linkName, zip, changedZip) {
    //alert("In trackLink");
    s_linkName = linkName; // linkName will hold "ZIP Code Changed"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }



    //alert('Zip changed from ' + zip);
    //alert('Zip changed to ' + changedZip);
    s.eVar11 = zip;             //Existing Zip
    s.eVar12 = changedZip;      //Changes Zip provided by user

    eventName = events[linkName]; // linkName will hold "ZIP Code Changed"

    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call

}

/* ------------------------------------------------------------
* 03.04 trackCompareShingles
* ------------------------------------------------------------
* DESC: Tracks the compare shingle event
* --------------------------------------------------------- */
function trackCompareShingles() {
    //alert("In trackLink");
    //s.sProp28 = jQuery.cookie("ProductNames").toString();
    //Will remove after confirmation
    //s.Prop28 = jQuery.cookie("ProductNames").toString();
    s.prop28 = jQuery.cookie("ProductNames").toString();
    //sProp28 = jQuery.cookie("ProductNames").toString();
    //
    //alert(jQuery.cookie("ProductNames").toString());
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    eventName = events["Compare Shingles"]; // linkName will hold "Compare Shingles"

    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call

}

/* ------------------------------------------------------------
* 03.05 trackOnSiteSearchEvent
* ------------------------------------------------------------
* DESC: Tracks the event , when user searches something on the site
* --------------------------------------------------------- */
function trackOnSiteSearchEvent(searchLabel, noOfResults) {
    var result;
    var yes = "y";
    var no = "n";
    if (noOfResults > 0) {
        result = yes;
    }
    else {
        result = no;
    }

    //    alert("In trackLink");
    //alert(searchLabel);
    //alert(result);

    s_linkName = "On-Site Search"; // linkName will hold "On-Site Search"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar31 = searchLabel;
    s.eVar32 = result;

    eventName = events["On-Site Search"];
    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset();
}
/* ------------------------------------------------------------
* 03.05 trackOnSiteSearchResultEvent
* ------------------------------------------------------------
* DESC: Tracks the event , when user searches something on the site
* --------------------------------------------------------- */
function trackOnSiteSearchResultEvent(searchTerm, title) {
    //    alert("In trackLink");
    //alert(searchTerm);
    //alert(linkName);

    s_linkName = "On-Site Search Results Click-Through"; // linkName will hold "On-Site Search"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar31 = searchTerm;
    s.eVar37 = title;
    s.eVar38 = searchTerm + " | " + title;
    eventName = events["On-Site Search Results Click-Through"];
    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset();
}
/* ------------------------------------------------------------
* 03.06 trackForm
* ------------------------------------------------------------
* DESC: Tracks the zip code change event
* --------------------------------------------------------- */
function trackForm(eventTriggered, formName) {
    //   alert(formName);
    //    alert(eventTriggered);

    s_linkName = eventTriggered; // eventTriggered will hold "Form Start  or Form Complete"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar15 = formName;

    eventName = events[eventTriggered];

    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call

}

/* ------------------------------------------------------------
* 03.07 trackOrderASampleEvent
* ------------------------------------------------------------
* DESC: Tracks the order a sample event.
* --------------------------------------------------------- */
//function trackOrderASampleEvent(linkName, color , product) {
function trackOrderASampleEvent(product) {
    //alert("In trackLink");
    //alert(color);
    //alert(product);

    s_linkName = "Order a Digital Sample"; // linkName will hold "Order a Digital Sample"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar24 = product;
    eventName = events["Order a Digital Sample"];

    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset(); // flush previous data after s.tl call
    //
}

/* ------------------------------------------------------------
* 03.08 trackWarrantyEvents
* ------------------------------------------------------------
* DESC: Tracks the event , when user searches something on the site
* --------------------------------------------------------- */
//function trackWarrantyEvents(stepName, linkName) {
    //alert("In trackLink");
    //alert(stepName);
    //alert(linkName);

//    s_linkName = linkName; // linkName will hold "On-Site Search"
  //  s_linkType = "o";



    // if (linkName == "Register a Warranty") {
//        s.eVar21 = stepName;
  //  }

//    else if (linkName == "File a Warranty Claim") {
  //      s.eVar22 = stepName
 //  }

 //   eventName = events[linkName];
//    if (eventName) { s.events = eventName; }

//    s.tl();

   // sDataReset();
//}
/* ------------------------------------------------------------
* 04.00 Page Inialization
* --------------------------------------------------------- */

/* ------------------------------------------------------------
* 03.09 trackShingleColorEvent()
* ------------------------------------------------------------
* DESC: Tracks the event , when user selects color for shingles on browse shingle screen
* --------------------------------------------------------- */
function trackShingleColorEvent(color, product) {
    //alert("In trackLink");
    //alert(color);
    //alert(product);

    s_linkName = "Select Color"; // linkName will hold "On-Site Search"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar17 = color;
    s.eVar18 = product;
    s.eVar19 = color + " | " + product;

    eventName = events["Select Color"];
    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset();
}

/* ------------------------------------------------------------
* 03.10 trackDesignYourOwnRoof()
* ------------------------------------------------------------
* DESC: Tracks the event , when user selects color for shingles on browse shingle screen
* --------------------------------------------------------- */
function trackDesignYourOwnRoof() {
    //alert("In trackLink");
    //alert(color);
    //alert(product);

    //s.eVar17 = "";
    //s.eVar18 = "";

    s_linkName = "Design Your Own Roof"; // linkName will hold "On-Site Search"
    s_linkType = "e";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    //if (color != undefined && color != "") {
    //  s.eVar17 = color;
    //}
    //if (product != undefined && product != "") {
    //  s.eVar18 = product;
    //}

    //    if (s.eVar17 != "" || s.eVar18 != "") {
    //      s.eVar19 = s.eVar17 + " | " + s.eVar18;
    // }


    eventName = events["Design Your Own Roof"];
    if (eventName) { s.events = eventName; }

    s.tl();

    sDataReset();
}


// change by nooopur
/* ------------------------------------------------------------
* 03.11 trackContactCall()
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks to call* --------------------------------------------------------- */
function trackContactCall(callType, Name) {
   // alert("In trackLink");
  //  alert(callType);
  //  alert(Name);

    s_linkName = "Click to Call"; 
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar20 = callType;
    s.eVar23 = unescape(Name);


    eventName = events["Click to Call"];
    eventName1 = events["Click to Call (Unique)"];
    eventName2 = events["Contact Events (Unique)"];
  //  if(eventName) {s.events=eventName;}
	//s.tl();
    
  //  if (eventName1) { s.events =eventName1; }
//	s.tl();
     
    if (eventName && eventName1 && eventName2) { s.events =eventName  +  "," + eventName1+"," + eventName2; }
    s.tl();
//	alert(eventName);
//	alert(eventName1);
//	alert(eventName2);
    sDataReset();
}
//end by noopur

/*------------------------Browse Shingles-------------------------*/
function trackBrowseShingless(product, activity) {
    //alert("In trackLink");
    //alert(activity);
    //alert(product);

    s_linkName = activity; // linkName will hold "On-Site Search"
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    s.eVar26 = activity;
    s.eVar27 = product;
    s.eVar28 = activity + " | " + product;

    eventName = events["Browse Shingles Event"];
    eventName1 = events["Browse Shingles Event (Unique)"];
	
    //alert(eventName);
    //alert(eventName1);
   // if (eventName) { s.events = eventName; }
    //alert(s.events);
  //  s.tl();
	if (eventName && eventName1) { s.events = eventName+ ","+ eventName1; }
	s.tl();

    //if (eventName1) { s.events = eventName1; }
    //alert(s.events);
    //s.tl();
    sDataReset();
}



// changed by noopur
/* ------------------------------------------------------------
* 03.12 trackSearchResultRefiningEvent()
* ------------------------------------------------------------
* DESC: Tracks the event , when user refines the serach result on the search page based on products and documents.
* --------------------------------------------------------- */
//function trackSearchResultRefiningEvent(sortType, sortSelection, documentType, productType) {
//    alert("In trackLink");
//    alert(sortType);
//    alert(sortSelection);
//    alert(documentType);
//    alert(productType);

//    s_linkName = "Sort Search Results";
//    s_linkType = "o";

//    s.eVar33 = sortType;
//    s.eVar34 = sortSelection;

//    if (sortType == "Document Type") {
//        s.eVar35 = documentType;
//    }
//    else if (sortType == "Product Type") {
//        s.eVar36 = productType;
//    }

//    eventName = events["Sort Search Results"];
//    if (eventName) { s.events = eventName; }

//    s.tl();

//    sDataReset();
//}
// end by noopur

/* ------------------------------------------------------------
* 03.13 trackPersonaSwitch
* ------------------------------------------------------------
* DESC: Global function that will be called by all pages to set s.PageName and s.campaign(if utm_id is there in the querystring) variable
* --------------------------------------------------------- */
function trackPages(pageToTrack, channel) {

    setCampaignVariable();
    var pageNameWithCategory = ""

    var zipcode = jQuery.cookie("UserInfoZip");
    if (zipcode != null) {
        var zip = zipcode.split("=")[1];
        s.prop3 = zip;
    }

    if (jQuery.cookie("category") != null) {
        s.prop1 = jQuery.cookie("category").toString().toLowerCase();
    }

    if (pageToTrack == "") {
        var pageName = (window.location.href).split('?')[0];
	pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, ""); 
        s.pageName = "oc";
      

        var pageNameObj = pageName.split("/");

        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();
                pageNameWithCategory += ' | ' + pageNameObj[j].toString().toLowerCase();
            }
        }
    }

    else {
        s.pageName = pageToTrack;
    }

	s.prop2 = "oc | " + s.prop1 + pageNameWithCategory;

    if (s.pageName == "oc" || s.pageName == "oc | #") {
        s.pageName = "oc" + " | " + "global homepage";
        s.prop1 = "global";
	  //change by noopur
        s.prop2 = "oc" + " | " + "global homepage";
    // end by noopur
	// s.prop2 = "oc" + " | "+ "global" + " | "  + "global homepage";
    }

   // if (s.pageName == "oc | roofing" || s.pageName == "oc | trumbull") {
  //      s.pageName += ' | ' + s.prop1;
    //}

    //s.prop2 = s.prop1 + '-' + s.pageName.toString();
    

    if (s.pageName.indexOf("oc | roofing") == 0) {
        s.channel = "roofing";
    }
    else if (s.pageName.indexOf("oc | trumbull") == 0) {
        s.channel = "trumbull";
    }
    else {
        s.channel = channel;
    }

        //alert(s.pageName);
        //alert(s.channel);
        //alert(s.prop1);
        //alert(s.prop2);
        //alert(s.prop3);



    s.t();
    sDataReset(); // flush previous data after s.tl call
}

/* ------------------------------------------------------------
* 03.14 setCampaignVariable()
* ------------------------------------------------------------
* DESC: Sets the campaign variable
* --------------------------------------------------------- */
function setCampaignVariable() {

    var querystring = location.search.replace('?', '').split('&');
    var queryObj = {};
    for (var i = 0; i < querystring.length; i++) {
        var name = querystring[i].split('=')[0];
        var value = querystring[i].split('=')[1];
        queryObj[name] = value;
    }
    if (queryObj["utm_id"]) {
        s.campaign = queryObj["utm_id"];
    }
}


// change by noopur
/* ------------------------------------------------------------
* 03.15 trackFindContractor()
* ------------------------------------------------------------
* DESC: Tracks the event , when user reaches the find a contractor page
* --------------------------------------------------------- */
function trackFindAContractor() {
  //  alert('In tracklink');
    var zipcode = jQuery.cookie("UserInfoZip");
    var zip = (zipcode != null ? zipcode.split("=")[1] : "");

    s_linkName = "Find a Contractor";
    s_linkType = "o";

	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
  if (s.pageName == "oc" || s.pageName == "oc | #") {
        s.pageName = "oc" + " | " + "global homepage";
}
    s.eVar16 = zip;
//    s.eVar17 = color;
//    s.eVar18 = product;
//    if (color == "" || color == null) {
//        s.eVar17 = "none";
//    }
//    if (product == "" || product == null) {
//        s.eVar18 = "none";
//    } 
//    s.eVar19 = color + ' | ' + product;
    if (localStorage && localStorage.getItem('contractorsForTracking')) {
        s.prop17 = localStorage.getItem('contractorsForTracking');        
    }    
  //  alert(s.prop17);
 //   alert(zip);
  eventName = events["Find a Contractor"];
eventName1 = events["Find a Contractor (Unique)"];
 eventName2 = events["Locate Events (Unique)"];
 //   if (eventName) { s.events = eventName; }
  //  s.tl();
    
   // if (eventName1) { s.events = eventName1 }
  //  s.tl();
   
    if (eventName && eventName1 && eventName2) { s.events = eventName + "," + eventName1 + "," + eventName2; }
    s.tl();
   // alert(eventName1);
    sDataReset();
}
// end by noopur

/* ------------------------------------------------------------
* 03.16 trackVideoViews()
* ------------------------------------------------------------
* DESC: Tracks the event , when user views a video.
* --------------------------------------------------------- */
function trackVieoViews(videoName, videoEvent) {

    //alert(videoName);
    //alert(videoEvent);

    s_linkName = videoEvent;
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar8 = videoName;


    eventName = events[videoEvent]; ;
    if (eventName) { s.events = eventName; }

    //alert(eventName);

    s.tl();

    sDataReset();
}

jQuery(document).ready(function ($) {

}); // end ready


// change by noopur    
/* ------------------------------------------------------------
* 03.17 trackRetailerDistributorInfo()
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on find a retail store or locate a distributor on browse shingle page.
* --------------------------------------------------------- */
function trackRetailerDistributorInfo(linkName) {
  //  alert("IntrackLink");
    var zipcode = jQuery.cookie("UserInfoZip");
    var zip = (zipcode != null ? zipcode.split("=")[1] : "");
    

    s_linkName = linkName;
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }
    s.eVar16 = zip;
//    s.eVar17 = color;
//    s.eVar18 = product;
//    if (color == "" || color == null) {
//        s.eVar17 = "none";
//    }
//    if (product == "" || product == null) {
//        s.eVar18 = "none";
//    }  
//        s.eVar19 = color + ' | ' + product;
    
    eventName = events[linkName];
eventName1 = events[linkName + "(Unique)"];
eventName2 = events["Locate Events (Unique)"];
  //  if (eventName) { s.events = eventName; }
  //  s.tl();

    
  //  if (eventName1) { s.events = eventName1 }
  //  s.tl();

    
   if (eventName && eventName1 && eventName2) { s.events = eventName + "," + eventName1 + "," + eventName2; }
    s.tl();

  //  alert(eventName2);
 //   alert(zip);
    sDataReset();
}
// end by noopur

/* ------------------------------------------------------------
* 03.18 trackSharedContent()
* ------------------------------------------------------------
* DESC: Tracks the event , when user shares the content on facebook or twitter
* --------------------------------------------------------- */
function trackSharedContent(platform, content) {


    //alert(platform);
    //alert(content);


    s_linkName = "Share";
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar13 = platform;
    s.eVar14 = content;

    eventName = events["Share"];
    if (eventName) { s.events = eventName; }


    s.tl();

    sDataReset();
}

//changed by noopur
/* ------------------------------------------------------------
* 03.18 trackRequestAQuote()
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on request a quote button
* --------------------------------------------------------- */
function trackRequestAQuote() {

    s_linkName = "Request a Quote";
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    if (jQuery.cookie("selectedContractorsName") != null) {
        s.prop24 = jQuery.cookie("selectedContractorsName").toString().replace(/\,/g, "");
        s.prop24 = s.prop24.toString().replace(/\|/g, ",");
    }

   
    var array = new Array();
    array = s.prop24.toString().split(',');
    var number = array.length;
  //  alert(number);
 //   alert(s.prop24);


    eventName = events["Request a Quote (numeric)"];
 eventName1 = events["Request a Quote (Unique)"];
 eventName2 = events["Contact Events (Unique)"];
  //  if (eventName) {s.events = eventName +"="+ number  ; }
  // s.tl();
   
  //  if (eventName1) { s.events = eventName1; }
  //  s.tl();
   
     if (eventName && eventName1 && eventName2) { s.events = eventName+"="+number+ "," + eventName1 + "," + eventName2; }
    s.tl();
 //   alert(eventName);
  //  alert(eventName1);
  //  alert(eventName2);
    sDataReset();
}
//end by noopur


/* ------------------------------------------------------------
* 03.18 trackStormBanner(eventText)
* ------------------------------------------------------------
* DESC: Tracks the event , when a storm popup appears or user click through
* --------------------------------------------------------- */
function trackStormBanner(eventText) {


    var zipcode = jQuery.cookie("UserInfoZip");
    var zip = (zipcode != null ? zipcode.split("=")[1] : "");
    //alert(zip);
    s_linkName =eventText;
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }
        s.eVar71 = zip;
    eventName = events[eventText];
     if (eventName) { s.events = eventName; }
    s.tl();
   

   

    sDataReset();

}

//added by noopur

/* ------------------------------------------------------------
* 03.19 trackContactRevealCall()
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on Call Us
* --------------------------------------------------------- */

function trackContactRevealCall(callType, Name) {
 //  alert(unescape(Name));
  //  alert("In trackLink");
 //   alert(callType);
 

    s_linkName = "Click to Reveal Phone Number";
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar20 = callType;
    s.eVar23 = unescape(Name);



    eventName = events["Click to Reveal Phone Number"];
    eventName1 = events["Click to Reveal Phone Number(Unique)"];
    eventName2 = events["Contact Events (Unique)"];

  //  if (eventName) { s.events = eventName}

  //  s.tl();
  //  if (eventName1) { s.events = eventName1 }

  //  s.tl();
    if (eventName && eventName1 && eventName2) { s.events = eventName + "," + eventName1 + "," + eventName2; }
    s.tl();
 //   alert(eventName2);
  //  alert(eventname1);


    sDataReset();
 //   alert(unescape(Name));
}

/* ------------------------------------------------------------
* 03.12 trackContractorRewardsApp()
* ------------------------------------------------------------
* DESC: Tracks the event , when Contractor Rewards Application link is clicked
* --------------------------------------------------------- */

function trackContractorRewardsApp() {
    
 // alert("In trackLink");
  

    s_linkName = "Contractor Rewards Application";
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    eventName = events["Contractor Rewards Application (Unique)"];
    eventName1 = events["Misc Events (Unique)"];
  

   // if (eventName) { s.events = eventName}

  //  s.tl();
  //  if (eventName1) { s.events = eventName1 }

   // s.tl();
 //   alert(eventName);	
 //   alert(eventname1);

if (eventName && eventName1) { s.events = eventName + "," + eventName1; }
    s.tl();
    sDataReset();
}

/* ------------------------------------------------------------
* 03.13 trackWarrantyClaimEvents
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on any warranty event
* --------------------------------------------------------- */
function trackWarrantyClaimEvents(linkName) {
  //    alert("In trackLink");
    //alert(stepName);
 //   alert(linkName);

    s_linkName = linkName; 
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar21 = linkName;

    eventName = events["Warranty Event"];
    evenName1 = events["Warranty Event (Unique)"];
   // if (eventName) { s.events = eventName}
  //  s.tl();
   // if (evenName1) { s.events = evenName1 }
   // s.tl();
 //   alert(evenName1);
if (eventName && evenName1) { s.events = eventName + "," + evenName1; }
    s.tl();
    sDataReset();
}

/* ------------------------------------------------------------
* 03.13 trackEmailContractorSubmission
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on any warranty event
* --------------------------------------------------------- */
function trackEmailContractorSubmission(storename) {
 //   alert("IntrackLink");
 //   alert(storename);
    s_linkName = "E-Mail Contractor";
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar23 = unescape(storename);

    eventName = events["E-Mail Contractor"];
    eventName1 = events["E-Mail Contractor (Unique)"];
    eventName2 = events["Contact Events (Unique)"];
   // if (eventName) { s.events = eventName }
   // s.tl();
   // if (evenName1) { s.events = evenName1 }
  //  s.tl();
  //  if (eventName2) { s.events = eventName2 }
   // s.tl();
 if (eventName && eventName1 && eventName2) { s.events = eventName + "," + eventName1 + "," + eventName2; }
    s.tl();
 //   alert(evenName1);
  //  alert(eventName2);
    sDataReset();
}

/* ------------------------------------------------------------
* 03.13 trackHelpGreen
* ------------------------------------------------------------
* DESC: Tracks the event , when user clicks on Help Grren This Contractor or Help Green My city
* --------------------------------------------------------- */
function trackHelpGreen(linkname) {
  //  alert("IntrackLink");
 //   alert(linkname);
	evenName1=null;
	eventName=null;
	
    s_linkName = linkname;
    s_linkType = "o";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }


    if (linkname == "Help Green This Contractor (Unique)") {
        eventName = events[linkname];
       // if (eventName) { s.events = eventName }
       // s.tl();
//alert(eventName);
    }
    if (linkname == "Help Green My City (Unique)") {
        evenName1 = events[linkname];
       // if (evenName1) { s.events = evenName1 }
       // s.tl();	
//alert(evenName1);
    }
    eventName2 = events["Misc Events (Unique)"];
  //  if (eventName2) { s.events = eventName2 }
   // s.tl(); 
//	alert(eventName2);
 if (eventName2 && evenName1) {s.events = eventName2+","+ evenName1;}

  if (eventName && eventName2) {s.events = eventName+","+ eventName2;}
    s.tl();
    sDataReset();
}

/* ------------------------------------------------------------
* 03.12 trackPDFDownload()
* ------------------------------------------------------------
* DESC: Tracks the event , when any PDF is downloaded
* --------------------------------------------------------- */

function trackPDFDownload(PdfName) {

   // alert("In trackLink");

   // alert(PdfName);
    s_linkName = "PDF Downloads";
    s_linkType = "d";
	  var pageName = (window.location.href).split('?')[0];
        pageName=pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");  
        s.pageName = "oc";
        var pageNameObj = pageName.split("/");
        for (var j = 3; j < pageNameObj.length; j++) {
            if (pageNameObj[j].toString() != "") {
                s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();               
            }
        }
        if (s.pageName == "oc" || s.pageName == "oc | #") {
            s.pageName = "oc" + " | " + "global homepage";
        }

    s.eVar62 = PdfName;

    eventName = events["PDF Downloads"];
    eventName1 = events["PDF Downloads (Unique)"];
    eventName2 = events["Misc Events (Unique)"];


  //  if (eventName) { s.events = eventName }

 //   s.tl();
//    if (eventName1) { s.events = eventName1 }

 //   s.tl();
  //  if (eventName2 ) { s.events = eventName2 }

 //   s.tl();

   // alert(eventname1);
 if (eventName && eventName1 && eventName2) {s.events = eventName+","+ eventName1+","+ eventName2;}

    s.tl();

    sDataReset();
}


//end by noopur


/* ------------------------------------------------------------
* 03.12 trackProductDetails(activity,productname)
* ------------------------------------------------------------
* DESC: Tracks the event , when any event is fired from product details page
* --------------------------------------------------------- */

function trackProductDetails(activity,productname)
{
    s_linkName = activity; 
    s_linkType = "o";
    var pageName = (window.location.href).split('?')[0];
    pageName = pageName.replace(/\#/g, "").replace(/\d+/g, "").replace(/\_/g, "").replace(/\/$/, "");
    s.pageName = "oc";
    var pageNameObj = pageName.split("/");
    for (var j = 3; j < pageNameObj.length; j++) {
        if (pageNameObj[j].toString() != "") {
            s.pageName += ' | ' + pageNameObj[j].toString().toLowerCase();
        }
    }
    if (s.pageName == "oc" || s.pageName == "oc | #") {
        s.pageName = "oc" + " | " + "global homepage";
    }
    s.eVar48 = activity;
    s.eVar49 = productname;
    s.eVar50 = activity + " | " + productname;
//alert(activity);
//alert(productname);
    eventName = events["Product Details Event"];
    eventName1 = events["Product Details Event (Unique)"];

    if (eventName && eventName1) { s.events = eventName + "," + eventName1; }
    s.tl();
    sDataReset();

}



/* ***************************************************************************************************************************************************************** */


/* ------------------------------------------------------------
* 05.00 Omniture Proprietary Code
* --------------------------------------------------------- */

//FOR PRODUCTION  
/*Write Session Cookie */
function s_vp_setSessionCookie(k, v) {
    var d = s.c_gd(); if (k) s_d.cookie = k + '=' + s.ape(v) + '; path=/;'
+ (d ? ' domain=' + d + ';' : ''); return s.c_r(k) == v
}

var previousEvents = s.c_r("s_p_events");
if (previousEvents.indexOf(s.events) == -1) { // If we haven't seen the event string before,
    previousEvents += s.events;  // Add the event to the cookie values
    s_vp_setSessionCookie("s_p_events", previousEvents); // Set the session cookie
} else { // Otherwise, we have seen the event string before.  Ignore the passed in values
    // s.events="";
    // s_products="";
}

/************* DO NOT ALTER ANYTHING BELOW THIS LINE ! **************/
var s_code = '', s_objectID; function s_gi(un, pg, ss) {
    var c = "s.version='H.24.3';s.an=s_an;s.logDebug=function(m){var s=this,tcf=new Function('var e;try{console.log(\"'+s.rep(s.rep(m,\"\\n\",\"\\\\n\"),\""
+ "\\\"\",\"\\\\\\\"\")+'\");}catch(e){}');tcf()};s.cls=function(x,c){var i,y='';if(!c)c=this.an;for(i=0;i<x.length;i++){n=x.substring(i,i+1);if(c.indexOf(n)>=0)y+=n}return y};s.fl=function(x,l){retur"
+ "n x?(''+x).substring(0,l):x};s.co=function(o){if(!o)return o;var n=new Object,x;for(x in o)if(x.indexOf('select')<0&&x.indexOf('filter')<0)n[x]=o[x];return n};s.num=function(x){x=''+x;for(var p=0;p"
+ "<x.length;p++)if(('0123456789').indexOf(x.substring(p,p+1))<0)return 0;return 1};s.rep=s_rep;s.sp=s_sp;s.jn=s_jn;s.ape=function(x){var s=this,h='0123456789ABCDEF',i,c=s.charSet,n,l,e,y='';c=c?c.toU"
+ "pperCase():'';if(x){x=''+x;if(s.em==3)x=encodeURIComponent(x);else if(c=='AUTO'&&('').charCodeAt){for(i=0;i<x.length;i++){c=x.substring(i,i+1);n=x.charCodeAt(i);if(n>127){l=0;e='';while(n||l<4){e=h"
+ ".substring(n%16,n%16+1)+e;n=(n-n%16)/16;l++}y+='%u'+e}else if(c=='+')y+='%2B';else y+=escape(c)}x=y}else x=escape(''+x);x=s.rep(x,'+','%2B');if(c&&c!='AUTO'&&s.em==1&&x.indexOf('%u')<0&&x.indexOf('"
+ "%U')<0){i=x.indexOf('%');while(i>=0){i++;if(h.substring(8).indexOf(x.substring(i,i+1).toUpperCase())>=0)return x.substring(0,i)+'u00'+x.substring(i);i=x.indexOf('%',i)}}}return x};s.epa=function(x)"
+ "{var s=this;if(x){x=s.rep(''+x,'+',' ');return s.em==3?decodeURIComponent(x):unescape(x)}return x};s.pt=function(x,d,f,a){var s=this,t=x,z=0,y,r;while(t){y=t.indexOf(d);y=y<0?t.length:y;t=t.substri"
+ "ng(0,y);r=s[f](t,a);if(r)return r;z+=y+d.length;t=x.substring(z,x.length);t=z<x.length?t:''}return ''};s.isf=function(t,a){var c=a.indexOf(':');if(c>=0)a=a.substring(0,c);c=a.indexOf('=');if(c>=0)a"
+ "=a.substring(0,c);if(t.substring(0,2)=='s_')t=t.substring(2);return (t!=''&&t==a)};s.fsf=function(t,a){var s=this;if(s.pt(a,',','isf',t))s.fsg+=(s.fsg!=''?',':'')+t;return 0};s.fs=function(x,f){var"
+ " s=this;s.fsg='';s.pt(x,',','fsf',f);return s.fsg};s.si=function(){var s=this,i,k,v,c=s_gi+'var s=s_gi(\"'+s.oun+'\");s.sa(\"'+s.un+'\");';for(i=0;i<s.va_g.length;i++){k=s.va_g[i];v=s[k];if(v!=unde"
+ "fined){if(typeof(v)!='number')c+='s.'+k+'=\"'+s_fe(v)+'\";';else c+='s.'+k+'='+v+';'}}c+=\"s.lnk=s.eo=s.linkName=s.linkType=s.wd.s_objectID=s.ppu=s.pe=s.pev1=s.pev2=s.pev3='';\";return c};s.c_d='';"
+ "s.c_gdf=function(t,a){var s=this;if(!s.num(t))return 1;return 0};s.c_gd=function(){var s=this,d=s.wd.location.hostname,n=s.fpCookieDomainPeriods,p;if(!n)n=s.cookieDomainPeriods;if(d&&!s.c_d){n=n?pa"
+ "rseInt(n):2;n=n>2?n:2;p=d.lastIndexOf('.');if(p>=0){while(p>=0&&n>1){p=d.lastIndexOf('.',p-1);n--}s.c_d=p>0&&s.pt(d,'.','c_gdf',0)?d.substring(p):d}}return s.c_d};s.c_r=function(k){var s=this;k=s.a"
+ "pe(k);var c=' '+s.d.cookie,i=c.indexOf(' '+k+'='),e=i<0?i:c.indexOf(';',i),v=i<0?'':s.epa(c.substring(i+2+k.length,e<0?c.length:e));return v!='[[B]]'?v:''};s.c_w=function(k,v,e){var s=this,d=s.c_gd"
+ "(),l=s.cookieLifetime,t;v=''+v;l=l?(''+l).toUpperCase():'';if(e&&l!='SESSION'&&l!='NONE'){t=(v!=''?parseInt(l?l:0):-60);if(t){e=new Date;e.setTime(e.getTime()+(t*1000))}}if(k&&l!='NONE'){s.d.cookie"
+ "=k+'='+s.ape(v!=''?v:'[[B]]')+'; path=/;'+(e&&l!='SESSION'?' expires='+e.toGMTString()+';':'')+(d?' domain='+d+';':'');return s.c_r(k)==v}return 0};s.eh=function(o,e,r,f){var s=this,b='s_'+e+'_'+s."
+ "_in,n=-1,l,i,x;if(!s.ehl)s.ehl=new Array;l=s.ehl;for(i=0;i<l.length&&n<0;i++){if(l[i].o==o&&l[i].e==e)n=i}if(n<0){n=i;l[n]=new Object}x=l[n];x.o=o;x.e=e;f=r?x.b:f;if(r||f){x.b=r?0:o[e];x.o[e]=f}if("
+ "x.b){x.o[b]=x.b;return b}return 0};s.cet=function(f,a,t,o,b){var s=this,r,tcf;if(s.apv>=5&&(!s.isopera||s.apv>=7)){tcf=new Function('s','f','a','t','var e,r;try{r=s[f](a)}catch(e){r=s[t](e)}return "
+ "r');r=tcf(s,f,a,t)}else{if(s.ismac&&s.u.indexOf('MSIE 4')>=0)r=s[b](a);else{s.eh(s.wd,'onerror',0,o);r=s[f](a);s.eh(s.wd,'onerror',1)}}return r};s.gtfset=function(e){var s=this;return s.tfs};s.gtfs"
+ "oe=new Function('e','var s=s_c_il['+s._in+'],c;s.eh(window,\"onerror\",1);s.etfs=1;c=s.t();if(c)s.d.write(c);s.etfs=0;return true');s.gtfsfb=function(a){return window};s.gtfsf=function(w){var s=thi"
+ "s,p=w.parent,l=w.location;s.tfs=w;if(p&&p.location!=l&&p.location.host==l.host){s.tfs=p;return s.gtfsf(s.tfs)}return s.tfs};s.gtfs=function(){var s=this;if(!s.tfs){s.tfs=s.wd;if(!s.etfs)s.tfs=s.cet"
+ "('gtfsf',s.tfs,'gtfset',s.gtfsoe,'gtfsfb')}return s.tfs};s.mrq=function(u){var s=this,l=s.rl[u],n,r;s.rl[u]=0;if(l)for(n=0;n<l.length;n++){r=l[n];s.mr(0,0,r.r,r.t,r.u)}};s.flushBufferedRequests=fun"
+ "ction(){};s.mr=function(sess,q,rs,ta,u){var s=this,dc=s.dc,t1=s.trackingServer,t2=s.trackingServerSecure,tb=s.trackingServerBase,p='.sc',ns=s.visitorNamespace,un=s.cls(u?u:(ns?ns:s.fun)),r=new Obje"
+ "ct,l,imn='s_i_'+(un),im,b,e;if(!rs){if(t1){if(t2&&s.ssl)t1=t2}else{if(!tb)tb='2o7.net';if(dc)dc=(''+dc).toLowerCase();else dc='d1';if(tb=='2o7.net'){if(dc=='d1')dc='112';else if(dc=='d2')dc='122';p"
+ "=''}t1=un+'.'+dc+'.'+p+tb}rs='http'+(s.ssl?'s':'')+'://'+t1+'/b/ss/'+s.un+'/'+(s.mobile?'5.1':'1')+'/'+s.version+(s.tcn?'T':'')+'/'+sess+'?AQB=1&ndh=1'+(q?q:'')+'&AQE=1';if(s.isie&&!s.ismac)rs=s.fl"
+ "(rs,2047)}if(s.d.images&&s.apv>=3&&(!s.isopera||s.apv>=7)&&(s.ns6<0||s.apv>=6.1)){if(!s.rc)s.rc=new Object;if(!s.rc[un]){s.rc[un]=1;if(!s.rl)s.rl=new Object;s.rl[un]=new Array;setTimeout('if(window"
+ ".s_c_il)window.s_c_il['+s._in+'].mrq(\"'+un+'\")',750)}else{l=s.rl[un];if(l){r.t=ta;r.u=un;r.r=rs;l[l.length]=r;return ''}imn+='_'+s.rc[un];s.rc[un]++}im=s.wd[imn];if(!im)im=s.wd[imn]=new Image;im."
+ "s_l=0;im.onload=new Function('e','this.s_l=1;var wd=window,s;if(wd.s_c_il){s=wd.s_c_il['+s._in+'];s.mrq(\"'+un+'\");s.nrs--;if(!s.nrs)s.m_m(\"rr\")}');if(!s.nrs){s.nrs=1;s.m_m('rs')}else s.nrs++;if"
+ "(s.debugTracking){var d='AppMeasurement Debug: '+rs,dl=s.sp(rs,'&'),dln;for(dln=0;dln<dl.length;dln++)d+=\"\\n\\t\"+s.epa(dl[dln]);s.logDebug(d)}im.src=rs;if((!ta||ta=='_self'||ta=='_top'||(s.wd.na"
+ "me&&ta==s.wd.name))&&rs.indexOf('&pe=')>=0){b=e=new Date;while(!im.s_l&&e.getTime()-b.getTime()<500)e=new Date}return ''}return '<im'+'g sr'+'c=\"'+rs+'\" width=1 height=1 border=0 alt=\"\">'};s.gg"
+ "=function(v){var s=this;if(!s.wd['s_'+v])s.wd['s_'+v]='';return s.wd['s_'+v]};s.glf=function(t,a){if(t.substring(0,2)=='s_')t=t.substring(2);var s=this,v=s.gg(t);if(v)s[t]=v};s.gl=function(v){var s"
+ "=this;if(s.pg)s.pt(v,',','glf',0)};s.rf=function(x){var s=this,y,i,j,h,p,l=0,q,a,b='',c='',t;if(x&&x.length>255){y=''+x;i=y.indexOf('?');if(i>0){q=y.substring(i+1);y=y.substring(0,i);h=y.toLowerCas"
+ "e();j=0;if(h.substring(0,7)=='http://')j+=7;else if(h.substring(0,8)=='https://')j+=8;i=h.indexOf(\"/\",j);if(i>0){h=h.substring(j,i);p=y.substring(i);y=y.substring(0,i);if(h.indexOf('google')>=0)l"
+ "=',q,ie,start,search_key,word,kw,cd,';else if(h.indexOf('yahoo.co')>=0)l=',p,ei,';if(l&&q){a=s.sp(q,'&');if(a&&a.length>1){for(j=0;j<a.length;j++){t=a[j];i=t.indexOf('=');if(i>0&&l.indexOf(','+t.su"
+ "bstring(0,i)+',')>=0)b+=(b?'&':'')+t;else c+=(c?'&':'')+t}if(b&&c)q=b+'&'+c;else c=''}i=253-(q.length-c.length)-y.length;x=y+(i>0?p.substring(0,i):'')+'?'+q}}}}return x};s.s2q=function(k,v,vf,vfp,f"
+ "){var s=this,qs='',sk,sv,sp,ss,nke,nk,nf,nfl=0,nfn,nfm;if(k==\"contextData\")k=\"c\";if(v){for(sk in v)if((!f||sk.substring(0,f.length)==f)&&v[sk]&&(!vf||vf.indexOf(','+(vfp?vfp+'.':'')+sk+',')>=0)"
+ "&&(!Object||!Object.prototype||!Object.prototype[sk])){nfm=0;if(nfl)for(nfn=0;nfn<nfl.length;nfn++)if(sk.substring(0,nfl[nfn].length)==nfl[nfn])nfm=1;if(!nfm){if(qs=='')qs+='&'+k+'.';sv=v[sk];if(f)"
+ "sk=sk.substring(f.length);if(sk.length>0){nke=sk.indexOf('.');if(nke>0){nk=sk.substring(0,nke);nf=(f?f:'')+nk+'.';if(!nfl)nfl=new Array;nfl[nfl.length]=nf;qs+=s.s2q(nk,v,vf,vfp,nf)}else{if(typeof(s"
+ "v)=='boolean'){if(sv)sv='true';else sv='false'}if(sv){if(vfp=='retrieveLightData'&&f.indexOf('.contextData.')<0){sp=sk.substring(0,4);ss=sk.substring(4);if(sk=='transactionID')sk='xact';else if(sk="
+ "='channel')sk='ch';else if(sk=='campaign')sk='v0';else if(s.num(ss)){if(sp=='prop')sk='c'+ss;else if(sp=='eVar')sk='v'+ss;else if(sp=='list')sk='l'+ss;else if(sp=='hier'){sk='h'+ss;sv=sv.substring("
+ "0,255)}}}qs+='&'+s.ape(sk)+'='+s.ape(sv)}}}}}if(qs!='')qs+='&.'+k}return qs};s.hav=function(){var s=this,qs='',l,fv='',fe='',mn,i,e;if(s.lightProfileID){l=s.va_m;fv=s.lightTrackVars;if(fv)fv=','+fv"
+ "+','+s.vl_mr+','}else{l=s.va_t;if(s.pe||s.linkType){fv=s.linkTrackVars;fe=s.linkTrackEvents;if(s.pe){mn=s.pe.substring(0,1).toUpperCase()+s.pe.substring(1);if(s[mn]){fv=s[mn].trackVars;fe=s[mn].tra"
+ "ckEvents}}}if(fv)fv=','+fv+','+s.vl_l+','+s.vl_l2;if(fe){fe=','+fe+',';if(fv)fv+=',events,'}if (s.events2)e=(e?',':'')+s.events2}for(i=0;i<l.length;i++){var k=l[i],v=s[k],b=k.substring(0,4),x=k.sub"
+ "string(4),n=parseInt(x),q=k;if(!v)if(k=='events'&&e){v=e;e=''}if(v&&(!fv||fv.indexOf(','+k+',')>=0)&&k!='linkName'&&k!='linkType'){if(k=='timestamp')q='ts';else if(k=='dynamicVariablePrefix')q='D';"
+ "else if(k=='visitorID')q='vid';else if(k=='pageURL'){q='g';v=s.fl(v,255)}else if(k=='referrer'){q='r';v=s.fl(s.rf(v),255)}else if(k=='vmk'||k=='visitorMigrationKey')q='vmt';else if(k=='visitorMigra"
+ "tionServer'){q='vmf';if(s.ssl&&s.visitorMigrationServerSecure)v=''}else if(k=='visitorMigrationServerSecure'){q='vmf';if(!s.ssl&&s.visitorMigrationServer)v=''}else if(k=='charSet'){q='ce';if(v.toUp"
+ "perCase()=='AUTO')v='ISO8859-1';else if(s.em==2||s.em==3)v='UTF-8'}else if(k=='visitorNamespace')q='ns';else if(k=='cookieDomainPeriods')q='cdp';else if(k=='cookieLifetime')q='cl';else if(k=='varia"
+ "bleProvider')q='vvp';else if(k=='currencyCode')q='cc';else if(k=='channel')q='ch';else if(k=='transactionID')q='xact';else if(k=='campaign')q='v0';else if(k=='resolution')q='s';else if(k=='colorDep"
+ "th')q='c';else if(k=='javascriptVersion')q='j';else if(k=='javaEnabled')q='v';else if(k=='cookiesEnabled')q='k';else if(k=='browserWidth')q='bw';else if(k=='browserHeight')q='bh';else if(k=='connec"
+ "tionType')q='ct';else if(k=='homepage')q='hp';else if(k=='plugins')q='p';else if(k=='events'){if(e)v+=(v?',':'')+e;if(fe)v=s.fs(v,fe)}else if(k=='events2')v='';else if(k=='contextData'){qs+=s.s2q('"
+ "c',s[k],fv,k,0);v=''}else if(k=='lightProfileID')q='mtp';else if(k=='lightStoreForSeconds'){q='mtss';if(!s.lightProfileID)v=''}else if(k=='lightIncrementBy'){q='mti';if(!s.lightProfileID)v=''}else "
+ "if(k=='retrieveLightProfiles')q='mtsr';else if(k=='deleteLightProfiles')q='mtsd';else if(k=='retrieveLightData'){if(s.retrieveLightProfiles)qs+=s.s2q('mts',s[k],fv,k,0);v=''}else if(s.num(x)){if(b="
+ "='prop')q='c'+n;else if(b=='eVar')q='v'+n;else if(b=='list')q='l'+n;else if(b=='hier'){q='h'+n;v=s.fl(v,255)}}if(v)qs+='&'+s.ape(q)+'='+(k.substring(0,3)!='pev'?s.ape(v):v)}}return qs};s.ltdf=funct"
+ "ion(t,h){t=t?t.toLowerCase():'';h=h?h.toLowerCase():'';var qi=h.indexOf('?');h=qi>=0?h.substring(0,qi):h;if(t&&h.substring(h.length-(t.length+1))=='.'+t)return 1;return 0};s.ltef=function(t,h){t=t?"
+ "t.toLowerCase():'';h=h?h.toLowerCase():'';if(t&&h.indexOf(t)>=0)return 1;return 0};s.lt=function(h){var s=this,lft=s.linkDownloadFileTypes,lef=s.linkExternalFilters,lif=s.linkInternalFilters;lif=li"
+ "f?lif:s.wd.location.hostname;h=h.toLowerCase();if(s.trackDownloadLinks&&lft&&s.pt(lft,',','ltdf',h))return 'd';if(s.trackExternalLinks&&h.substring(0,1)!='#'&&(lef||lif)&&(!lef||s.pt(lef,',','ltef'"
+ ",h))&&(!lif||!s.pt(lif,',','ltef',h)))return 'e';return ''};s.lc=new Function('e','var s=s_c_il['+s._in+'],b=s.eh(this,\"onclick\");s.lnk=s.co(this);s.t();s.lnk=0;if(b)return this[b](e);return true"
+ "');s.bc=new Function('e','var s=s_c_il['+s._in+'],f,tcf;if(s.d&&s.d.all&&s.d.all.cppXYctnr)return;s.eo=e.srcElement?e.srcElement:e.target;tcf=new Function(\"s\",\"var e;try{if(s.eo&&(s.eo.tagName||"
+ "s.eo.parentElement||s.eo.parentNode))s.t()}catch(e){}\");tcf(s);s.eo=0');s.oh=function(o){var s=this,l=s.wd.location,h=o.href?o.href:'',i,j,k,p;i=h.indexOf(':');j=h.indexOf('?');k=h.indexOf('/');if"
+ "(h&&(i<0||(j>=0&&i>j)||(k>=0&&i>k))){p=o.protocol&&o.protocol.length>1?o.protocol:(l.protocol?l.protocol:'');i=l.pathname.lastIndexOf('/');h=(p?p+'//':'')+(o.host?o.host:(l.host?l.host:''))+(h.subs"
+ "tring(0,1)!='/'?l.pathname.substring(0,i<0?0:i)+'/':'')+h}return h};s.ot=function(o){var t=o.tagName;if(o.tagUrn||(o.scopeName&&o.scopeName.toUpperCase()!='HTML'))return '';t=t&&t.toUpperCase?t.toU"
+ "pperCase():'';if(t=='SHAPE')t='';if(t){if((t=='INPUT'||t=='BUTTON')&&o.type&&o.type.toUpperCase)t=o.type.toUpperCase();else if(!t&&o.href)t='A';}return t};s.oid=function(o){var s=this,t=s.ot(o),p,c"
+ ",n='',x=0;if(t&&!o.s_oid){p=o.protocol;c=o.onclick;if(o.href&&(t=='A'||t=='AREA')&&(!c||!p||p.toLowerCase().indexOf('javascript')<0))n=s.oh(o);else if(c){n=s.rep(s.rep(s.rep(s.rep(''+c,\"\\r\",''),"
+ "\"\\n\",''),\"\\t\",''),' ','');x=2}else if(t=='INPUT'||t=='SUBMIT'){if(o.value)n=o.value;else if(o.innerText)n=o.innerText;else if(o.textContent)n=o.textContent;x=3}else if(o.src&&t=='IMAGE')n=o.s"
+ "rc;if(n){o.s_oid=s.fl(n,100);o.s_oidt=x}}return o.s_oid};s.rqf=function(t,un){var s=this,e=t.indexOf('='),u=e>=0?t.substring(0,e):'',q=e>=0?s.epa(t.substring(e+1)):'';if(u&&q&&(','+u+',').indexOf('"
+ ",'+un+',')>=0){if(u!=s.un&&s.un.indexOf(',')>=0)q='&u='+u+q+'&u=0';return q}return ''};s.rq=function(un){if(!un)un=this.un;var s=this,c=un.indexOf(','),v=s.c_r('s_sq'),q='';if(c<0)return s.pt(v,'&'"
+ ",'rqf',un);return s.pt(un,',','rq',0)};s.sqp=function(t,a){var s=this,e=t.indexOf('='),q=e<0?'':s.epa(t.substring(e+1));s.sqq[q]='';if(e>=0)s.pt(t.substring(0,e),',','sqs',q);return 0};s.sqs=functi"
+ "on(un,q){var s=this;s.squ[un]=q;return 0};s.sq=function(q){var s=this,k='s_sq',v=s.c_r(k),x,c=0;s.sqq=new Object;s.squ=new Object;s.sqq[q]='';s.pt(v,'&','sqp',0);s.pt(s.un,',','sqs',q);v='';for(x i"
+ "n s.squ)if(x&&(!Object||!Object.prototype||!Object.prototype[x]))s.sqq[s.squ[x]]+=(s.sqq[s.squ[x]]?',':'')+x;for(x in s.sqq)if(x&&(!Object||!Object.prototype||!Object.prototype[x])&&s.sqq[x]&&(x==q"
+ "||c<2)){v+=(v?'&':'')+s.sqq[x]+'='+s.ape(x);c++}return s.c_w(k,v,0)};s.wdl=new Function('e','var s=s_c_il['+s._in+'],r=true,b=s.eh(s.wd,\"onload\"),i,o,oc;if(b)r=this[b](e);for(i=0;i<s.d.links.leng"
+ "th;i++){o=s.d.links[i];oc=o.onclick?\"\"+o.onclick:\"\";if((oc.indexOf(\"s_gs(\")<0||oc.indexOf(\".s_oc(\")>=0)&&oc.indexOf(\".tl(\")<0)s.eh(o,\"onclick\",0,s.lc);}return r');s.wds=function(){var s"
+ "=this;if(s.apv>3&&(!s.isie||!s.ismac||s.apv>=5)){if(s.b&&s.b.attachEvent)s.b.attachEvent('onclick',s.bc);else if(s.b&&s.b.addEventListener)s.b.addEventListener('click',s.bc,false);else s.eh(s.wd,'o"
+ "nload',0,s.wdl)}};s.vs=function(x){var s=this,v=s.visitorSampling,g=s.visitorSamplingGroup,k='s_vsn_'+s.un+(g?'_'+g:''),n=s.c_r(k),e=new Date,y=e.getYear();e.setYear(y+10+(y<1900?1900:0));if(v){v*="
+ "100;if(!n){if(!s.c_w(k,x,e))return 0;n=x}if(n%10000>v)return 0}return 1};s.dyasmf=function(t,m){if(t&&m&&m.indexOf(t)>=0)return 1;return 0};s.dyasf=function(t,m){var s=this,i=t?t.indexOf('='):-1,n,"
+ "x;if(i>=0&&m){var n=t.substring(0,i),x=t.substring(i+1);if(s.pt(x,',','dyasmf',m))return n}return 0};s.uns=function(){var s=this,x=s.dynamicAccountSelection,l=s.dynamicAccountList,m=s.dynamicAccoun"
+ "tMatch,n,i;s.un=s.un.toLowerCase();if(x&&l){if(!m)m=s.wd.location.host;if(!m.toLowerCase)m=''+m;l=l.toLowerCase();m=m.toLowerCase();n=s.pt(l,';','dyasf',m);if(n)s.un=n}i=s.un.indexOf(',');s.fun=i<0"
+ "?s.un:s.un.substring(0,i)};s.sa=function(un){var s=this;s.un=un;if(!s.oun)s.oun=un;else if((','+s.oun+',').indexOf(','+un+',')<0)s.oun+=','+un;s.uns()};s.m_i=function(n,a){var s=this,m,f=n.substrin"
+ "g(0,1),r,l,i;if(!s.m_l)s.m_l=new Object;if(!s.m_nl)s.m_nl=new Array;m=s.m_l[n];if(!a&&m&&m._e&&!m._i)s.m_a(n);if(!m){m=new Object,m._c='s_m';m._in=s.wd.s_c_in;m._il=s._il;m._il[m._in]=m;s.wd.s_c_in"
+ "++;m.s=s;m._n=n;m._l=new Array('_c','_in','_il','_i','_e','_d','_dl','s','n','_r','_g','_g1','_t','_t1','_x','_x1','_rs','_rr','_l');s.m_l[n]=m;s.m_nl[s.m_nl.length]=n}else if(m._r&&!m._m){r=m._r;r"
+ "._m=m;l=m._l;for(i=0;i<l.length;i++)if(m[l[i]])r[l[i]]=m[l[i]];r._il[r._in]=r;m=s.m_l[n]=r}if(f==f.toUpperCase())s[n]=m;return m};s.m_a=new Function('n','g','e','if(!g)g=\"m_\"+n;var s=s_c_il['+s._"
+ "in+'],c=s[g+\"_c\"],m,x,f=0;if(!c)c=s.wd[\"s_\"+g+\"_c\"];if(c&&s_d)s[g]=new Function(\"s\",s_ft(s_d(c)));x=s[g];if(!x)x=s.wd[\\'s_\\'+g];if(!x)x=s.wd[g];m=s.m_i(n,1);if(x&&(!m._i||g!=\"m_\"+n)){m."
+ "_i=f=1;if((\"\"+x).indexOf(\"function\")>=0)x(s);else s.m_m(\"x\",n,x,e)}m=s.m_i(n,1);if(m._dl)m._dl=m._d=0;s.dlt();return f');s.m_m=function(t,n,d,e){t='_'+t;var s=this,i,x,m,f='_'+t,r=0,u;if(s.m_"
+ "l&&s.m_nl)for(i=0;i<s.m_nl.length;i++){x=s.m_nl[i];if(!n||x==n){m=s.m_i(x);u=m[t];if(u){if((''+u).indexOf('function')>=0){if(d&&e)u=m[t](d,e);else if(d)u=m[t](d);else u=m[t]()}}if(u)r=1;u=m[t+1];if"
+ "(u&&!m[f]){if((''+u).indexOf('function')>=0){if(d&&e)u=m[t+1](d,e);else if(d)u=m[t+1](d);else u=m[t+1]()}}m[f]=1;if(u)r=1}}return r};s.m_ll=function(){var s=this,g=s.m_dl,i,o;if(g)for(i=0;i<g.lengt"
+ "h;i++){o=g[i];if(o)s.loadModule(o.n,o.u,o.d,o.l,o.e,1);g[i]=0}};s.loadModule=function(n,u,d,l,e,ln){var s=this,m=0,i,g,o=0,f1,f2,c=s.h?s.h:s.b,b,tcf;if(n){i=n.indexOf(':');if(i>=0){g=n.substring(i+"
+ "1);n=n.substring(0,i)}else g=\"m_\"+n;m=s.m_i(n)}if((l||(n&&!s.m_a(n,g)))&&u&&s.d&&c&&s.d.createElement){if(d){m._d=1;m._dl=1}if(ln){if(s.ssl)u=s.rep(u,'http:','https:');i='s_s:'+s._in+':'+n+':'+g;"
+ "b='var s=s_c_il['+s._in+'],o=s.d.getElementById(\"'+i+'\");if(s&&o){if(!o.l&&s.wd.'+g+'){o.l=1;if(o.i)clearTimeout(o.i);o.i=0;s.m_a(\"'+n+'\",\"'+g+'\"'+(e?',\"'+e+'\"':'')+')}';f2=b+'o.c++;if(!s.m"
+ "axDelay)s.maxDelay=250;if(!o.l&&o.c<(s.maxDelay*2)/100)o.i=setTimeout(o.f2,100)}';f1=new Function('e',b+'}');tcf=new Function('s','c','i','u','f1','f2','var e,o=0;try{o=s.d.createElement(\"script\""
+ ");if(o){o.type=\"text/javascript\";'+(n?'o.id=i;o.defer=true;o.onload=o.onreadystatechange=f1;o.f2=f2;o.l=0;':'')+'o.src=u;c.appendChild(o);'+(n?'o.c=0;o.i=setTimeout(f2,100)':'')+'}}catch(e){o=0}r"
+ "eturn o');o=tcf(s,c,i,u,f1,f2)}else{o=new Object;o.n=n+':'+g;o.u=u;o.d=d;o.l=l;o.e=e;g=s.m_dl;if(!g)g=s.m_dl=new Array;i=0;while(i<g.length&&g[i])i++;g[i]=o}}else if(n){m=s.m_i(n);m._e=1}return m};"
+ "s.voa=function(vo,r){var s=this,l=s.va_g,i,k,v,x;for(i=0;i<l.length;i++){k=l[i];v=vo[k];if(v||vo['!'+k]){if(!r&&(k==\"contextData\"||k==\"retrieveLightData\")&&s[k])for(x in s[k])if(!v[x])v[x]=s[k]"
+ "[x];s[k]=v}}};s.vob=function(vo){var s=this,l=s.va_g,i,k;for(i=0;i<l.length;i++){k=l[i];vo[k]=s[k];if(!vo[k])vo['!'+k]=1}};s.dlt=new Function('var s=s_c_il['+s._in+'],d=new Date,i,vo,f=0;if(s.dll)f"
+ "or(i=0;i<s.dll.length;i++){vo=s.dll[i];if(vo){if(!s.m_m(\"d\")||d.getTime()-vo._t>=s.maxDelay){s.dll[i]=0;s.t(vo)}else f=1}}if(s.dli)clearTimeout(s.dli);s.dli=0;if(f){if(!s.dli)s.dli=setTimeout(s.d"
+ "lt,s.maxDelay)}else s.dll=0');s.dl=function(vo){var s=this,d=new Date;if(!vo)vo=new Object;s.vob(vo);vo._t=d.getTime();if(!s.dll)s.dll=new Array;s.dll[s.dll.length]=vo;if(!s.maxDelay)s.maxDelay=250"
+ ";s.dlt()};s.track=s.t=function(vo){var s=this,trk=1,tm=new Date,sed=Math&&Math.random?Math.floor(Math.random()*10000000000000):tm.getTime(),sess='s'+Math.floor(tm.getTime()/10800000)%10+sed,y=tm.ge"
+ "tYear(),vt=tm.getDate()+'/'+tm.getMonth()+'/'+(y<1900?y+1900:y)+' '+tm.getHours()+':'+tm.getMinutes()+':'+tm.getSeconds()+' '+tm.getDay()+' '+tm.getTimezoneOffset(),tcf,tfs=s.gtfs(),ta=-1,q='',qs='"
+ "',code='',vb=new Object;s.gl(s.vl_g);s.uns();s.m_ll();if(!s.td){var tl=tfs.location,a,o,i,x='',c='',v='',p='',bw='',bh='',j='1.0',k=s.c_w('s_cc','true',0)?'Y':'N',hp='',ct='',pn=0,ps;if(String&&Str"
+ "ing.prototype){j='1.1';if(j.match){j='1.2';if(tm.setUTCDate){j='1.3';if(s.isie&&s.ismac&&s.apv>=5)j='1.4';if(pn.toPrecision){j='1.5';a=new Array;if(a.forEach){j='1.6';i=0;o=new Object;tcf=new Funct"
+ "ion('o','var e,i=0;try{i=new Iterator(o)}catch(e){}return i');i=tcf(o);if(i&&i.next)j='1.7'}}}}}if(s.apv>=4)x=screen.width+'x'+screen.height;if(s.isns||s.isopera){if(s.apv>=3){v=s.n.javaEnabled()?'"
+ "Y':'N';if(s.apv>=4){c=screen.pixelDepth;bw=s.wd.innerWidth;bh=s.wd.innerHeight}}s.pl=s.n.plugins}else if(s.isie){if(s.apv>=4){v=s.n.javaEnabled()?'Y':'N';c=screen.colorDepth;if(s.apv>=5){bw=s.d.doc"
+ "umentElement.offsetWidth;bh=s.d.documentElement.offsetHeight;if(!s.ismac&&s.b){tcf=new Function('s','tl','var e,hp=0;try{s.b.addBehavior(\"#default#homePage\");hp=s.b.isHomePage(tl)?\"Y\":\"N\"}cat"
+ "ch(e){}return hp');hp=tcf(s,tl);tcf=new Function('s','var e,ct=0;try{s.b.addBehavior(\"#default#clientCaps\");ct=s.b.connectionType}catch(e){}return ct');ct=tcf(s)}}}else r=''}if(s.pl)while(pn<s.pl"
+ ".length&&pn<30){ps=s.fl(s.pl[pn].name,100)+';';if(p.indexOf(ps)<0)p+=ps;pn++}s.resolution=x;s.colorDepth=c;s.javascriptVersion=j;s.javaEnabled=v;s.cookiesEnabled=k;s.browserWidth=bw;s.browserHeight"
+ "=bh;s.connectionType=ct;s.homepage=hp;s.plugins=p;s.td=1}if(vo){s.vob(vb);s.voa(vo)}if((vo&&vo._t)||!s.m_m('d')){if(s.usePlugins)s.doPlugins(s);var l=s.wd.location,r=tfs.document.referrer;if(!s.pag"
+ "eURL)s.pageURL=l.href?l.href:l;if(!s.referrer&&!s._1_referrer){s.referrer=r;s._1_referrer=1}s.m_m('g');if(s.lnk||s.eo){var o=s.eo?s.eo:s.lnk,p=s.pageName,w=1,t=s.ot(o),n=s.oid(o),x=o.s_oidt,h,l,i,o"
+ "c;if(s.eo&&o==s.eo){while(o&&!n&&t!='BODY'){o=o.parentElement?o.parentElement:o.parentNode;if(o){t=s.ot(o);n=s.oid(o);x=o.s_oidt}}if(!n||t=='BODY')o='';if(o){oc=o.onclick?''+o.onclick:'';if((oc.ind"
+ "exOf('s_gs(')>=0&&oc.indexOf('.s_oc(')<0)||oc.indexOf('.tl(')>=0)o=0}}if(o){if(n)ta=o.target;h=s.oh(o);i=h.indexOf('?');h=s.linkLeaveQueryString||i<0?h:h.substring(0,i);l=s.linkName;t=s.linkType?s."
+ "linkType.toLowerCase():s.lt(h);if(t&&(h||l)){s.pe='lnk_'+(t=='d'||t=='e'?t:'o');s.pev1=(h?s.ape(h):'');s.pev2=(l?s.ape(l):'')}else trk=0;if(s.trackInlineStats){if(!p){p=s.pageURL;w=0}t=s.ot(o);i=o."
+ "sourceIndex;if(s.gg('objectID')){n=s.gg('objectID');x=1;i=1}if(p&&n&&t)qs='&pid='+s.ape(s.fl(p,255))+(w?'&pidt='+w:'')+'&oid='+s.ape(s.fl(n,100))+(x?'&oidt='+x:'')+'&ot='+s.ape(t)+(i?'&oi='+i:'')}}"
+ "else trk=0}if(trk||qs){s.sampled=s.vs(sed);if(trk){if(s.sampled)code=s.mr(sess,(vt?'&t='+s.ape(vt):'')+s.hav()+q+(qs?qs:s.rq()),0,ta);qs='';s.m_m('t');if(s.p_r)s.p_r();s.referrer=s.lightProfileID=s"
+ ".retrieveLightProfiles=s.deleteLightProfiles=''}s.sq(qs)}}else s.dl(vo);if(vo)s.voa(vb,1);s.lnk=s.eo=s.linkName=s.linkType=s.wd.s_objectID=s.ppu=s.pe=s.pev1=s.pev2=s.pev3='';if(s.pg)s.wd.s_lnk=s.wd"
+ ".s_eo=s.wd.s_linkName=s.wd.s_linkType='';return code};s.trackLink=s.tl=function(o,t,n,vo){var s=this;s.lnk=s.co(o);s.linkType=t;s.linkName=n;s.t(vo)};s.trackLight=function(p,ss,i,vo){var s=this;s.l"
+ "ightProfileID=p;s.lightStoreForSeconds=ss;s.lightIncrementBy=i;s.t(vo)};s.setTagContainer=function(n){var s=this,l=s.wd.s_c_il,i,t,x,y;s.tcn=n;if(l)for(i=0;i<l.length;i++){t=l[i];if(t&&t._c=='s_l'&"
+ "&t.tagContainerName==n){s.voa(t);if(t.lmq)for(i=0;i<t.lmq.length;i++){x=t.lmq[i];y='m_'+x.n;if(!s[y]&&!s[y+'_c']){s[y]=t[y];s[y+'_c']=t[y+'_c']}s.loadModule(x.n,x.u,x.d)}if(t.ml)for(x in t.ml)if(s["
+ "x]){y=s[x];x=t.ml[x];for(i in x)if(!Object.prototype[i]){if(typeof(x[i])!='function'||(''+x[i]).indexOf('s_c_il')<0)y[i]=x[i]}}if(t.mmq)for(i=0;i<t.mmq.length;i++){x=t.mmq[i];if(s[x.m]){y=s[x.m];if"
+ "(y[x.f]&&typeof(y[x.f])=='function'){if(x.a)y[x.f].apply(y,x.a);else y[x.f].apply(y)}}}if(t.tq)for(i=0;i<t.tq.length;i++)s.t(t.tq[i]);t.s=s;return}}};s.wd=window;s.ssl=(s.wd.location.protocol.toLow"
+ "erCase().indexOf('https')>=0);s.d=document;s.b=s.d.body;if(s.d.getElementsByTagName){s.h=s.d.getElementsByTagName('HEAD');if(s.h)s.h=s.h[0]}s.n=navigator;s.u=s.n.userAgent;s.ns6=s.u.indexOf('Netsca"
+ "pe6/');var apn=s.n.appName,v=s.n.appVersion,ie=v.indexOf('MSIE '),o=s.u.indexOf('Opera '),i;if(v.indexOf('Opera')>=0||o>0)apn='Opera';s.isie=(apn=='Microsoft Internet Explorer');s.isns=(apn=='Netsc"
+ "ape');s.isopera=(apn=='Opera');s.ismac=(s.u.indexOf('Mac')>=0);if(o>0)s.apv=parseFloat(s.u.substring(o+6));else if(ie>0){s.apv=parseInt(i=v.substring(ie+5));if(s.apv>3)s.apv=parseFloat(i)}else if(s"
+ ".ns6>0)s.apv=parseFloat(s.u.substring(s.ns6+10));else s.apv=parseFloat(v);s.em=0;if(s.em.toPrecision)s.em=3;else if(String.fromCharCode){i=escape(String.fromCharCode(256)).toUpperCase();s.em=(i=='%"
+ "C4%80'?2:(i=='%U0100'?1:0))}if(s.oun)s.sa(s.oun);s.sa(un);s.vl_l='timestamp,dynamicVariablePrefix,visitorID,vmk,visitorMigrationKey,visitorMigrationServer,visitorMigrationServerSecure,ppu,charSet,v"
+ "isitorNamespace,cookieDomainPeriods,cookieLifetime,pageName,pageURL,referrer,contextData,currencyCode,lightProfileID,lightStoreForSeconds,lightIncrementBy,retrieveLightProfiles,deleteLightProfiles,"
+ "retrieveLightData';s.va_l=s.sp(s.vl_l,',');s.vl_mr=s.vl_m='timestamp,charSet,visitorNamespace,cookieDomainPeriods,cookieLifetime,contextData,lightProfileID,lightStoreForSeconds,lightIncrementBy';s."
+ "vl_t=s.vl_l+',variableProvider,channel,server,pageType,transactionID,purchaseID,campaign,state,zip,events,events2,products,linkName,linkType';var n;for(n=1;n<=75;n++){s.vl_t+=',prop'+n+',eVar'+n;s."
+ "vl_m+=',prop'+n+',eVar'+n}for(n=1;n<=5;n++)s.vl_t+=',hier'+n;for(n=1;n<=3;n++)s.vl_t+=',list'+n;s.va_m=s.sp(s.vl_m,',');s.vl_l2=',tnt,pe,pev1,pev2,pev3,resolution,colorDepth,javascriptVersion,javaE"
+ "nabled,cookiesEnabled,browserWidth,browserHeight,connectionType,homepage,plugins';s.vl_t+=s.vl_l2;s.va_t=s.sp(s.vl_t,',');s.vl_g=s.vl_t+',trackingServer,trackingServerSecure,trackingServerBase,fpCo"
+ "okieDomainPeriods,disableBufferedRequests,mobile,visitorSampling,visitorSamplingGroup,dynamicAccountSelection,dynamicAccountList,dynamicAccountMatch,trackDownloadLinks,trackExternalLinks,trackInlin"
+ "eStats,linkLeaveQueryString,linkDownloadFileTypes,linkExternalFilters,linkInternalFilters,linkTrackVars,linkTrackEvents,linkNames,lnk,eo,lightTrackVars,_1_referrer,un';s.va_g=s.sp(s.vl_g,',');s.pg="
+ "pg;s.gl(s.vl_g);s.contextData=new Object;s.retrieveLightData=new Object;if(!ss)s.wds();if(pg){s.wd.s_co=function(o){s_gi(\"_\",1,1).co(o)};s.wd.s_gs=function(un){s_gi(un,1,1).t()};s.wd.s_dc=functio"
+ "n(un){s_gi(un,1).t()}}",
w = window, l = w.s_c_il, n = navigator, u = n.userAgent, v = n.appVersion, e = v.indexOf('MSIE '), m = u.indexOf('Netscape6/'), a, i, j, x, s; if (un) { un = un.toLowerCase(); if (l) for (j = 0; j < 2; j++) for (i = 0; i < l.length; i++) { s = l[i]; x = s._c; if ((!x || x == 's_c' || (j > 0 && x == 's_l')) && (s.oun == un || (s.fs && s.sa && s.fs(s.oun, un)))) { if (s.sa) s.sa(un); if (x == 's_c') return s } else s = 0 } } w.s_an = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    w.s_sp = new Function("x", "d", "var a=new Array,i=0,j;if(x){if(x.split)a=x.split(d);else if(!d)for(i=0;i<x.length;i++)a[a.length]=x.substring(i,i+1);else while(i>=0){j=x.indexOf(d,i);a[a.length]=x.subst"
+ "ring(i,j<0?x.length:j);i=j;if(i>=0)i+=d.length}}return a");
    w.s_jn = new Function("a", "d", "var x='',i,j=a.length;if(a&&j>0){x=a[0];if(j>1){if(a.join)x=a.join(d);else for(i=1;i<j;i++)x+=d+a[i]}}return x");
    w.s_rep = new Function("x", "o", "n", "return s_jn(s_sp(x,o),n)");
    w.s_d = new Function("x", "var t='`^@$#',l=s_an,l2=new Object,x2,d,b=0,k,i=x.lastIndexOf('~~'),j,v,w;if(i>0){d=x.substring(0,i);x=x.substring(i+2);l=s_sp(l,'');for(i=0;i<62;i++)l2[l[i]]=i;t=s_sp(t,'');d"
+ "=s_sp(d,'~');i=0;while(i<5){v=0;if(x.indexOf(t[i])>=0) {x2=s_sp(x,t[i]);for(j=1;j<x2.length;j++){k=x2[j].substring(0,1);w=t[i]+k;if(k!=' '){v=1;w=d[b+l2[k]]}x2[j]=w+x2[j].substring(1)}}if(v)x=s_jn("
+ "x2,'');else{w=t[i]+' ';if(x.indexOf(w)>=0)x=s_rep(x,w,t[i]);i++;b+=62}}}return x");
    w.s_fe = new Function("c", "return s_rep(s_rep(s_rep(c,'\\\\','\\\\\\\\'),'\"','\\\\\"'),\"\\n\",\"\\\\n\")");
    w.s_fa = new Function("f", "var s=f.indexOf('(')+1,e=f.indexOf(')'),a='',c;while(s>=0&&s<e){c=f.substring(s,s+1);if(c==',')a+='\",\"';else if((\"\\n\\r\\t \").indexOf(c)<0)a+=c;s++}return a?'\"'+a+'\"':"
+ "a");
    w.s_ft = new Function("c", "c+='';var s,e,o,a,d,q,f,h,x;s=c.indexOf('=function(');while(s>=0){s++;d=1;q='';x=0;f=c.substring(s);a=s_fa(f);e=o=c.indexOf('{',s);e++;while(d>0){h=c.substring(e,e+1);if(q){i"
+ "f(h==q&&!x)q='';if(h=='\\\\')x=x?0:1;else x=0}else{if(h=='\"'||h==\"'\")q=h;if(h=='{')d++;if(h=='}')d--}if(d>0)e++}c=c.substring(0,s)+'new Function('+(a?a+',':'')+'\"'+s_fe(c.substring(o+1,e))+'\")"
+ "'+c.substring(e+1);s=c.indexOf('=function(')}return c;");
    c = s_d(c); if (e > 0) { a = parseInt(i = v.substring(e + 5)); if (a > 3) a = parseFloat(i) } else if (m > 0) a = parseFloat(u.substring(m + 10)); else a = parseFloat(v); if (a < 5 || v.indexOf('Opera') >= 0 || u.indexOf('Opera') >= 0) c = s_ft(c); if (!s) { s = new Object; if (!w.s_c_in) { w.s_c_il = new Array; w.s_c_in = 0 } s._il = w.s_c_il; s._in = w.s_c_in; s._il[s._in] = s; w.s_c_in++; } s._c = 's_c'; (new Function("s", "un", "pg", "ss", c))(s, un, pg, ss); return s
}
function s_giqf() { var w = window, q = w.s_giq, i, t, s; if (q) for (i = 0; i < q.length; i++) { t = q[i]; s = s_gi(t.oun); s.sa(t.un); s.setTagContainer(t.tagContainerName) } w.s_giq = 0 } s_giqf()


